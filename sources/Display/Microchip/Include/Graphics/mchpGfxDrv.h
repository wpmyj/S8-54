...
#define RCC_RSY                             0x64096001ul//0x64001096ul
...
void SetColorN(void);
void FillRectN(SHORT x, SHORT y, SHORT width, SHORT height);
void MemoryTrans(void);
void VerLineN(SHORT x, SHORT y1, SHORT y2);
void HorLineN(SHORT x1, SHORT x2, SHORT y);
void PutPixelN(SHORT x, SHORT y);
void mSetPaletteColor(WORD color,BYTE number);
void SetWorkAreasN(void);
void TextOutN(SHORT x, SHORT y, UXCHAR *str, DWORD addr);
...
WORD mSetArea(SHORT left, SHORT top, SHORT width,SHORT height,DWORD srcOffsetAddr, BYTE curColor);
