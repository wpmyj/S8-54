...
void HAL_PCD_SetupStageCallback(PCD_HandleTypeDef *hpcd)
{
    ...
    {                                                               //
        ...
        {                                                           //
            if (prevLength != 0)                                    //
            {                                                       //
                gConnectToHost = true;                              // GOVNOCODE “аким вот замысловатым образом определ€ем, что к нам подконнектилс€ хост (
            }                                                       //
            else                                                    //
            {                                                       //
...
void HAL_PCD_DataInStageCallback(PCD_HandleTypeDef *hpcd, uint8_t epnum)
{
    ...
    // WARN здесь оказываем€ после каждой передачи пакета. ћожно отловить момент, когда передача закончена.
}
