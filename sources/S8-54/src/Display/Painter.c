...
void Painter_LoadPalette(int num)
{
    ...
    for (; i <= a; i++)
    {
        ...
        Timer_PauseOnTime(1);           // WARN Эта задержака вставлена потому, что без неё при загрузке палитры иногда подмигивает экран.
...
#pragma pack(4)
    ...
    for (int y = 239; y >= 0; y--)
    {
        for (int x = 1; x < 320; x += 2)
        {
            ...
            while (color != Read2points(x, y))      // Этот цикл нужен потому, что иногда цвета считываются неправильно - проверка, что повторное чтение читает тот же цвет.
            {
