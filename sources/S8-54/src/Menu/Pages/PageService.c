...
static const      Choice cEthernet_Ethernet;                        ///< СЕРВИС - ETHERNET - Ethernet
static void     OnChanged_Ethernet_Settings(bool active);
static const  IPaddress ipEthernet_IP;                              ///< СЕРВИС - ETHERNET - IP адрес
static const  IPaddress ipEthernet_NetMask;                         ///< СЕРВИС - ETHERNET - Маска подсети
...
static const       Page ppSound;                                    ///< СЕРВИС - ЗВУК
static const      Choice cSound_Enable;                             ///< СЕРВИС - ЗВУК - Звук
static const    Governor gSound_Volume;                             ///< СЕРВИС - ЗВУК - Громкость
static const       Page ppTime;                                     ///< СЕРВИС - ВРЕМЯ
static const        Time tTime_Time;                                ///< СЕРВИС - ВРЕМЯ - Время
static const    Governor tTime_Correction;                          ///< СЕРВИС - ВРЕМЯ - Коррекция
static void     OnChanged_Time_Correction(void);
static const      Choice cLanguage;                                 ///< СЕРВИС - ЯЗЫК
static const       Page ppInformation;                              ///< СЕРВИС - ИНФОРМАЦИЯ
static void       OnPress_Information(void);
static void   Information_Draw(void);
static const     SButton bInformation_Exit;                         ///< СЕРВИС - ИНФОРМАЦИЯ - Выход
...
static const     SButton bRecorder_Start;                           ///< СЕРВИС - РЕГИСТРАТОР - Старт/Стоп
...
static const Page ppTime =
{
    ...
    Page_Service_Time,
    {
        (void*)&tTime_Time,         // СЕРВИС - ВРЕМЯ - Время
        (void*)&tTime_Correction    // CЕРВИС - ВРЕМЯ - Коррекция
    }
};
