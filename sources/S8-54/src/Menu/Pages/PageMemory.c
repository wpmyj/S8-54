...
static const   SButton bLast_Exit;                              ///< ПАМЯТЬ - Последние - Выход
static void     OnPress_Last_Exit(void);
static const   SButton bLast_Next;                              ///< ПАМЯТЬ - Последние - Следующий
static void     OnPress_Last_Next(void);
static void        Draw_Last_Next(int x, int y);
static const   SButton bLast_Prev;                              ///< ПАМЯТЬ - Последние - Предыдущий
static void     OnPress_Last_Prev(void);
static void        Draw_Last_Prev(int x, int y);
static const   SButton bLast_SaveToROM;                         ///< ПАМЯТЬ - Последние - Внутр ЗУ
static void     OnPress_Last_SaveToROM(void);
static void        Draw_Last_SaveToROM(int x, int y);
static const   SButton bLast_SaveToDrive;                       ///< ПАМЯТЬ - Последние - Сохранить
...
static const   SButton bInternal_Exit;                          ///< ПАМЯТЬ - ВНУТР ЗУ - Выход
static void     OnPress_Internal_Exit(void);
static const   SButton bInternal_ShowAlways;                    ///< ПАМЯТЬ - ВНУТР ЗУ - Показывать всегда
...
static const   SButton bInternal_ModeShow;                      ///< ПАМЯТЬ - ВНУТР ЗУ - Вид сигнала
...
static const   SButton bInternal_Delete;                        ///< ПАМЯТЬ - ВНУТР ЗУ - Удалить
...
*/
static const   SButton bInternal_Scale;                         ///< ПАМЯТЬ - ВНУТР ЗУ - Месштаб
...
static const   SButton bInternal_SaveToDrive;                   ///< ПАМЯТЬ - ВНУТР ЗУ - Сохранить на флешку
...
static const   SButton bDrive_Manager_Exit;                     ///< ПАМЯТЬ - ВНЕШН ЗУ - КАТАЛОГ - Выход
static void     OnPress_Drive_Manager_Exit(void);
static const   SButton bDrive_Manager_Tab;                      ///< ПАМЯТЬ - ВНЕШН ЗУ - КАТАЛОГ - Tab
static void        Draw_Drive_Manager_Tab(int x, int y);
static const   SButton bDrive_Manager_LevelUp;                  ///< ПАМЯТЬ - ВНЕШН ЗУ - КАТАЛОГ - Выйти из каталога
static void        Draw_Drive_Manager_LevelUp(int x, int y);
static const   SButton bDrive_Manager_LevelDown;                ///< ПАМЯТЬ - ВНЕШН ЗУ - КАТАЛОГ - Войти в каталог
...
static const   SButton bDrive_Mask_Exit;                        ///< ПАМЯТЬ - ВНЕШН ЗУ - МАСКА - Выход
static const   SButton bDrive_Mask_Delete;                      ///< ПАМЯТЬ - ВНЕШН ЗУ - МАСКА - Удалить
static void     OnPress_Drive_Mask_Delete(void);
static void        Draw_Drive_Mask_Delete(int x, int y);
static const   SButton bDrive_Mask_Backspace;                   ///< ПАМЯТЬ - ВНЕШН ЗУ - МАСКА - Backspace
static void     OnPress_Drive_Mask_Backspace(void);
static void        Draw_Drive_Mask_Backspace(int x, int y);
static const   SButton bDrive_Mask_Insert;                      ///< ПАМЯТЬ - ВНЕШН ЗУ - МАСКА - Вставить
...
static const   SButton bSetName_Exit;                           ///< УСТАНОВКА ИМЕНИ - Выход
static void     OnPress_SetName_Exit(void);
static const   SButton bSetName_Delete;                         ///< УСТАНОВКА ИМЕНИ - Удалить
static void     OnPress_SetName_Delete(void);
static void        Draw_SetName_Delete(int x, int y);
static const   SButton bSetName_Backspace;                      ///< УСТАНОВКА ИМЕНИ - Backspace
static void     OnPress_SetName_Backspace(void);
static void        Draw_SetName_Backspace(int x, int y);
static const   SButton bSetName_Insert;                         ///< УСТАНОВКА ИМЕНИ - Вставить
static void     OnPress_SetName_Insert(void);
static void        Draw_SetName_Insert(int x, int y);
static const   SButton bSetName_Save;                           ///< УСТАНОВКА ИМЕНИ - Сохранить
