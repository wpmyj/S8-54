...
#ifdef _MS_VS
#pragma warning(push)
#pragma warning(disable:4129)   // warning C4129: 'C' : unrecognized character escape sequence
#endif
...
void ShortPress_Page(void *item)
{
    ...
    if (set.display.showStringNavigation == ShowStrNavi_Temp)       // Если нужно временно выводить строку навигации меню
    {
        TemporaryEnableStrNavi();
    }
    SetCurrentItem(page, true);
...
static bool NeedForFireSetLED(void)    // Возвращает true, если лампочка УСТАНОВКА должна гореть
{
    ...
    if (
            ...
            (name == Page_SB_MathFunction && MATH_NEED_DRAW)
        )
    {
        return true;
    }
