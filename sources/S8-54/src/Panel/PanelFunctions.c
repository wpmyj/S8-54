...
{
    int count = CalculateCount(prevTime);
    int trigLevOld = TRIGLEV(trigSource);
    int trigLev = TRIGLEV(trigSource) + relStep * count;
...
void ChangeTShift(int *prevTime, void(*f)(int), int16 relStep)
{
    ...
    int16 tShift = TSHIFT + (int16)step;
...
{
    static int prevTime = 0;
    if (!FPGA_IsRunning() || set.time.timeDivXPos == FunctionTime_ShiftInMemory)
    {
