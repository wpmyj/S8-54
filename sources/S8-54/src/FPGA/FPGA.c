...
{
    if (!WORK_DIRECT)     // Если находимся не в режиме непосредственного считывания сигнала
    {
        return;
    }
    ...
    if (sTime_P2PModeEnabled())
    {
        if (Timer2_IsRun(kTimerStartP2P))                // Если находимся в режиме поточечного вывода и в данный момент пауза после считывания очередного полного сигнала
        {
            Timer2_Disable(kTimerStartP2P);              // То останавливаем таймер, чтобы просмотреть сигнал
        }
        else
        {
            FPGA_Start();
