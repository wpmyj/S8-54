...
#if defined (GFX_USE_DISPLAY_CONTROLLER_MCHP_DA210)//ExternalMemoryCallback
...
volatile DWORD 		_DisplayBuffer;
volatile DWORD      _FrameBuffer;
...
/*#if   (((GFX_EPMP_CS1_MEMORY_SIZE) > 0x10000ul)  && ((GFX_EPMP_CS1_MEMORY_SIZE) <= 0x20000ul))
		...
		PMCS1CFbits.PTSZ = 0;      //0 							// data bus width is 16-bit 
...
/************ End - Project Specific Code - Relocate ************/
     ...
     _DisplayBuffer=GFX_WORK_AREA1_START_ADDRESS;
    _FrameBuffer=GFX_WORK_AREA2_START_ADDRESS;
...
void PutPixel(SHORT x, SHORT y)
{
    ...
    #else
        GFX_SetWorkArea2(GFX_WORK_AREA1_START_ADDRESS); 
    #endif
...
WORD OutCharEds(XCHAR ch)
{
	...
	{   
		...
		GFX_SetWorkArea1(_drawbuffer);
	#else
		GFX_SetWorkArea1(GFX_WORK_AREA1_START_ADDRESS);
	#endif
...
//----MY PALETTE FUNC
void mSetPalette(PALETTE_ENTRY *pPaletteEntry,WORD length)
{
  WORD counter;
  _CLUTRWEN = 1;
...
void mSetPaletteColor(WORD _color,BYTE _number)
{
  WORD counter=(WORD)_number;
         
	   while(_VMRGN==0) Nop();	 
		 _CLUTRWEN = 1;
         _CLUTADR = counter;//+counter;
        _CLUTWR  = _color;//pPaletteEntry[counter].value;
		//ppal++;
        while(_CLUTBUSY)
        {
            Nop();
...
#ifdef USE_MY_PRIMITIVIES

void FillWorkArea(void)
{
    GFX_WaitForCommandQueue(GFX_COMMAND_QUEUE_LENGTH);
    GFX_WaitForGpu();
	
	GFX_SetWorkArea2(GFX_WORK_AREA1_START_ADDRESS);//2 
...
////////////////////////////////////////////////////////////
void MemoryTrans(void)
{
      ROPBlockInline(GFX_WORK_AREA2_START_ADDRESS, GFX_WORK_AREA1_START_ADDRESS, 0, 0, GFX_DATA_CONTINUOUS, GFX_DATA_DISCONTINUOUS, 
              RCC_COPY, RCC_ROP_C,  0,  DISP_HOR_RESOLUTION, DISP_VER_RESOLUTION);
		//ROPBlockInline(_FrameBuffer, _FrameBuffer, 0, 0, GFX_DATA_CONTINUOUS, GFX_DATA_DISCONTINUOUS, 
         //      RCC_COPY, RCC_ROP_C,  0,  DISP_HOR_RESOLUTION, DISP_VER_RESOLUTION);
}
void mMemoryTrans(void)
{
      ROPBlockInline(GFX_WORK_AREA2_START_ADDRESS, GFX_WORK_AREA1_START_ADDRESS, 0, 0, GFX_DATA_CONTINUOUS, GFX_DATA_DISCONTINUOUS, 
               RCC_TRANSPARENT_COPY, RCC_ROP_E,  0,  DISP_HOR_RESOLUTION, DISP_VER_RESOLUTION);
}

////////////////////////////////////////////////////////////
WORD mLineBarX(SHORT left, SHORT top, SHORT right, SHORT bottom)
{
	...
	if(GFX_GetFreeCommandSpace() < GFX_COMMAND_QUEUE_LENGTH)
        ...
        GFX_SetWorkArea2(GFX_WORK_AREA2_START_ADDRESS); 
...
WORD mSetLineBarX(DWORD addr, SHORT right, SHORT bottom)
{
...
//    SHORT width, height;
    ...
    GFX_SetWorkArea2(srcOffsetAddr); 
        
    GFX_RCC_SetColor(GetColor());
    GFX_RCC_SetDestOffset(0);
    GFX_RCC_SetSize(10, 1);
    GFX_RCC_StartCopy(RCC_SOLID_FILL, RCC_ROP_C, RCC_SRC_ADDR_DISCONTINUOUS, RCC_DEST_ADDR_CONTINUOUS);
...
///////////////////////////////////////////////////////////
WORD mSetArea(SHORT left, SHORT top, SHORT width,SHORT height,DWORD srcOffsetAddr, BYTE _curr_color)
{


//ROPBlock(0,GFX_WORK_AREA2_START_ADDRESS,srcOffsetAddr,((top * (DWORD)DISP_HOR_RESOLUTION) + left),0,0,1,12,1,16,1);
   DWORD_VAL workArea1Temp, workArea2Temp;
    DWORD dstOffsetAddr;
...
GFX_WaitForCommandQueue(GFX_COMMAND_QUEUE_LENGTH);
	...
	//if(GFX_GetFreeCommandSpace() < GFX_COMMAND_QUEUE_LENGTH)
      //  {
        ...
        dstOffsetAddr = (top * (DWORD)DISP_HOR_RESOLUTION) + left;
    
        GFX_SetWorkArea1(srcOffsetAddr);
        GFX_SetWorkArea2(GFX_WORK_AREA2_START_ADDRESS); 
        GFX_RCC_SetSrcOffset(0);
        GFX_RCC_SetColor(0);
        GFX_RCC_SetDestOffset(dstOffsetAddr);
        GFX_RCC_SetSize( width,height);
        GFX_RCC_StartCopy(RCC_TRANSPARENT_COPY, RCC_ROP_C, RCC_SRC_ADDR_CONTINUOUS, RCC_DEST_ADDR_DISCONTINUOUS);
	...
	SetColor(_curr_color);
	SetColorN();
	
    return (1);
}

WORD mSetAreaN(SHORT left, SHORT top, SHORT width,SHORT height,DWORD srcOffsetAddr)
{
...
////////////////
WORD vLineBarX(SHORT left, SHORT top, SHORT right, SHORT bottom)
{
...
///////////////////////////////////////////////////////////
void vPutPixel(SHORT x, SHORT y)
{
   
    
       
   
    //GFX_SetWorkArea2(_FrameBuffer);//(GFX_WORK_AREA2_START_ADDRESS); 
...
////////////////////////////////////////////////////////////
void mPutPixel(SHORT x, SHORT y)
{
...
{
  GFX_SetWorkArea2(GFX_WORK_AREA2_START_ADDRESS); 
  GFX_RCC_SetColor(GetColor());GFX_RCC_SetSize(1, 1);
}
////////////////////////////////////////////////////////////
WORD GraphLine(SHORT x1, SHORT y1, SHORT x2, SHORT y2)
{
   SHORT   temp,length;
   
   if((x1 == x2) || (y1 == y2))
      {
	     vLineBarX(x1, y1, x2, y2);
		 return(1);
      }
	  else
	  {
	     if(y1 > y2)
          { 
			...
			switch(temp)
			{
			  case 1:  vPutPixel(x1, y1);
		               vPutPixel(x2, y2);
			           break;
			  case 2:  vLineBarX(x1, y1-1,x1, y1 );
		               //vLineBarX(x2, y2, x2, y2+1);
					   vPutPixel(x2, y2);
			           break;
			  default: length=temp/2;
		               vLineBarX( x1, y1-(temp-length-1),x1, y1);
		               vLineBarX(x2, y2, x2, y2+length);
			           break;
			}
            return(1);
		   }
		 ...
		 {  
		    ...
		    switch(temp)
			{
			  case 1:  vPutPixel(x1, y1);
		               vPutPixel(x2, y2);
			           break;
			  case 2:  vLineBarX(x2, y2-1,x2, y2 );
			           //vLineBarX(x1, y1, x1, y1+1);
		               vPutPixel(x1, y1);
			           break;
			  default: length=temp/2;
		               vLineBarX(x1, y1, x1, y1+length);
		               vLineBarX( x2, y2-(temp-length-1),x2, y2);
			           break;
			}
		    return(1);
		 }
...
void DrawGraph(SHORT x_st,SHORT dy,SHORT num, SHORT *dots)//UINT16 dot[300])//
{ 
  //SetColor(CLR14);
  //SetColor(10);
 // SetWorkArea();
  SHORT   temp,length,y1,y2;
  UINT16 i,x1,x2;
  x1=x_st;
  
  for (i=0;i<num-1;i++)
   {  
      y1=*(dots+i)+dy;//dot[i]+dy;
	  y2=*(dots+i+1)+dy;//dot[i+1]+dy;
      x2=x1+1;
	  //UARTPutChar(y1);
	  //UARTPutChar(y2);
      if((x1 == x2) || (y1 == y2))
...
////////////////////////////////////////////////////////////
WORD mLine(SHORT x1, SHORT y1, SHORT x2, SHORT y2)
{   
    ...
    {   
        ...
        if(_lineThickness)
        {
            if(x1 == x2)
            {
                if(!vLineBarX(x1 - 1, y1, x2 + 1, y2)) return (0);
            }
            else
            {
                if(!vLineBarX(x1, y1 - 1, x2, y2 + 1)) return (0);
            }
        }
        else
        {
            if(!vLineBarX(x1, y1, x2, y2)) return (0);
        }
...
void vFillWorkArea(DWORD Layer)
{
    GFX_WaitForCommandQueue(GFX_COMMAND_QUEUE_LENGTH);
    GFX_WaitForGpu();
	
	GFX_SetWorkArea2(Layer); 
...
////////////////////////////////////////////////////////////
void vMemoryTrans(void)
{     
      //vFillWorkArea(DisplayBuffer);
      ROPBlockInline(GFX_WORK_AREA2_START_ADDRESS, GFX_WORK_AREA1_START_ADDRESS, 0, 0, GFX_DATA_CONTINUOUS, GFX_DATA_CONTINUOUS, 
            RCC_COPY, RCC_ROP_C,  0,  DISP_HOR_RESOLUTION, DISP_VER_RESOLUTION);
    //ROPBlockInline(Layer1, WorkLayer, 0, 0, GFX_DATA_CONTINUOUS, GFX_DATA_CONTINUOUS, 
    //         RCC_TRANSPARENT_COPY, RCC_ROP_C,  0,  DISP_HOR_RESOLUTION, DISP_VER_RESOLUTION);
	//ROPBlockInline(Layer3, WorkLayer, 0, 0, GFX_DATA_CONTINUOUS, GFX_DATA_CONTINUOUS, 
    //             RCC_TRANSPARENT_COPY, RCC_ROP_C,  0,  DISP_HOR_RESOLUTION, DISP_VER_RESOLUTION); 
			   // ROPBlockInline(WorkLayer, GFX_WORK_AREA1_START_ADDRESS, 0, 0, GFX_DATA_CONTINUOUS, GFX_DATA_DISCONTINUOUS, 
     //          RCC_COPY, RCC_ROP_C,  0,  DISP_HOR_RESOLUTION, DISP_VER_RESOLUTION);
}
////////////////////////////////////////////////////////////
WORD vLineBar(SHORT left, SHORT top, SHORT right, SHORT bottom, DWORD Layer)
{
...
////////////////////////////////////////////////////////////
/*void vPutPixel(SHORT x, SHORT y, DWORD Layer)
{
...
////////////////////////////////////////////////////////////
/*WORD vLine(SHORT x1, SHORT y1, SHORT x2, SHORT y2,DWORD Layer)
{
    SHORT   deltaX, deltaY;
    SHORT   error, stepErrorLT, stepErrorGE;
    SHORT   stepX, stepY;
    ...
    if((_lineType == 0) && ((x1 == x2) || (y1 == y2)))
    {
        ...
        if(_lineThickness)
        {
            if(x1 == x2)
            {
                if(!vLineBar(x1 - 1, y1, x2 + 1, y2, Layer)) return (0);
            }
            else
            {
                if(!vLineBar(x1, y1 - 1, x2, y2 + 1, Layer)) return (0);
            }
        }
        else
        {
            if(!vLineBar(x1, y1, x2, y2, Layer)) return (0);
        }
    ...
    if(x1 == x2)
    {
        ...
        for(temp = y1; temp < y2 + 1; temp++)
        {
            ...
            if(type)
            {
                vPutPixel(x1, temp,Layer);
                if(_lineThickness)
                {
                    vPutPixel(x1 + 1, temp, Layer);
                    vPutPixel(x1 - 1, temp, Layer);
                }
            }
        }
    ...
    if(y1 == y2)
    {
        ...
        for(temp = x1; temp < x2 + 1; temp++)
        {
            ...
            if(type)
            {
                vPutPixel(temp, y1, Layer);
                if(_lineThickness)
                {
                    vPutPixel(temp, y1 + 1, Layer);
                    vPutPixel(temp, y1 - 1, Layer);
                }
            }
        }
    ...
    if(deltaX < deltaY)
    {
        ...
        stepX = stepY;
        stepY = temp;
        vPutPixel(y1, x1,Layer);
    }
    else
    {
        vPutPixel(x1, y1,Layer);
    }
...
DWORD vGetPixel(SHORT x, SHORT y)
{
    DWORD_VAL workArea1Temp,workArea2Temp;
    volatile DWORD getcolor = 0;
...
workArea2Temp.w[1] = G1W2ADRH;
    //#ifdef USE_DOUBLE_BUFFERING
      //  GFX_SetWorkArea1(_drawbuffer);
    //#else
        GFX_SetWorkArea1(GFX_WORK_AREA2_START_ADDRESS);
    //#endif    
    ...
    GFX_RCC_SetDestOffset(0);
    GFX_RCC_SetSize(8, 1);
    GFX_RCC_StartCopy(RCC_COPY, RCC_ROP_C, RCC_SRC_ADDR_CONTINUOUS, RCC_DEST_ADDR_DISCONTINUOUS);//DIS
...
G1W2ADRH = workArea2Temp.w[1];
    ...
    return (DWORD)getcolor;
}
			   
void NewOutChar(UXCHAR ch)
{ 
    while(IsDeviceBusy() != 0) {Nop();}
	...
	{  // mLineBarX(35+_i,10,35+_i,25);
		...
		GFX_SetWorkArea1(GFX_WORK_AREA2_START_ADDRESS);//(GFX_WORK_AREA1_START_ADDRESS);//
...
void NewOutTextXY(SHORT _x, SHORT _y, UXCHAR *_textString,DWORD _addr)
{ 
   UXCHAR   _ch;
   MoveTo(_x, _y);
   //SHORT _i=0;
   GFX_CHR_SetFont(_addr);
  	
    while((_ch = *_textString++) != 0)//((UXCHAR)15 < (UXCHAR)(_ch = *_textString++)) //Nop();
       //while(NewOutChar(_ch) == 0);
       NewOutChar(_ch);
}


void CopyImageRam(void *image,BYTE *dest)
{
int i;
GFX_IMAGE_HEADER *piHdr;
piHdr = (GFX_IMAGE_HEADER*)image;
...
for(i=0; i < piHdr->param1; i++)
                {
                    //aTemp[i] = pFlashData[i];
                    *dest = *pFlashData;
                    pFlashData++;
                    dest++;  
...
void mCopyImageRam(void *image,DWORD dest)
{
...
DWORD srcaddress = (DWORD)(WORD)aTemp;
...
ROPBlock(srcaddress, dest, 0, 0, GFX_DATA_CONTINUOUS, GFX_DATA_CONTINUOUS, RCC_COPY, RCC_ROP_C, 0, 881, 1);
GFX_WaitForCommandQueue(GFX_COMMAND_QUEUE_LENGTH);
            GFX_WaitForGpu();
...
void DrawImageRam(void *image,DWORD src,DWORD dest)
{
         GFX_IMAGE_HEADER *piHdr;
         piHdr = (GFX_IMAGE_HEADER*)image;
            ...
            GFX_SetWorkArea1(src);
            
            GFX_SetWorkArea2(dest);
            GFX_IPU_SetSrcOffset(0);
            GFX_IPU_SetDestOffset(0);
            GFX_IPU_Inflate(piHdr->param2);
...
void PutPixelN(SHORT _x, SHORT _y)
{
    GFX_SetWorkArea2(GFX_WORK_AREA2_START_ADDRESS); // GFX_SetWorkArea2(GFX_WORK_AREA2_START_ADDRESS);
    ...
    GFX_WaitForCommandQueue(4);////????????????????????????????????????????????????
    GFX_RCC_SetColor(GetColor());
	GFX_RCC_SetDestOffset((_y * (DWORD)DISP_HOR_RESOLUTION) + _x);// delet -1
	GFX_RCC_SetSize(1, 1);
	GFX_RCC_StartCopy(RCC_SOLID_FILL, RCC_ROP_C, RCC_SRC_ADDR_DISCONTINUOUS, RCC_DEST_ADDR_CONTINUOUS); //RCC_DEST_ADDR_DISCONTINUOUS
	
	//---GFX_WaitForCommandQueue(GFX_COMMAND_QUEUE_LENGTH);
         //  ----- GFX_WaitForGpu();
}

/*void VerDotLineN(SHORT _x, SHORT _y,SHORT _offset,SHORT _num)
{
    GFX_SetWorkArea2(GFX_WORK_AREA2_START_ADDRESS);
    GFX_WaitForCommandQueue(1);
    GFX_RCC_SetSize(1, 1);
...
for
	GFX_RCC_SetDestOffset((_y * (DWORD)DISP_HOR_RESOLUTION) + _x);
	
	GFX_RCC_StartCopy(RCC_SOLID_FILL, RCC_ROP_C, RCC_SRC_ADDR_DISCONTINUOUS, RCC_DEST_ADDR_DISCONTINUOUS);
}*/

void FillRectN(SHORT _x, SHORT _y, SHORT _width, SHORT _height)
{

	DWORD srcOffsetAddr;
	srcOffsetAddr = (_y * (DWORD)DISP_HOR_RESOLUTION) + _x;
    ...
    //GFX_RCC_SetColor(GetColor());
	GFX_RCC_SetDestOffset(srcOffsetAddr);
	GFX_RCC_SetSize((_width+1), (_height+1));
	GFX_RCC_StartCopy(RCC_SOLID_FILL, RCC_ROP_C, RCC_SRC_ADDR_DISCONTINUOUS, RCC_DEST_ADDR_DISCONTINUOUS);//?? GFX_RCC_StartCopy(RCC_SOLID_FILL, RCC_ROP_C, RCC_SRC_ADDR_DISCONTINUOUS, RCC_DEST_ADDR_DISCONTINUOUS);
...
void VerLineN(SHORT x, SHORT y1, SHORT y2)
{
	...
	{
	   height = y1 - y2 + 1;
       srcOffsetAddr = (y2 * (DWORD)DISP_HOR_RESOLUTION) + x;
	}
	else 
	{
	   height = y2 - y1 + 1;
       srcOffsetAddr = (y1 * (DWORD)DISP_HOR_RESOLUTION) + x;
	}
	...
	//GFX_RCC_SetColor(GetColor());
	GFX_RCC_SetDestOffset(srcOffsetAddr);
	GFX_RCC_SetSize(1, height);
	GFX_RCC_StartCopy(RCC_SOLID_FILL, RCC_ROP_C, RCC_SRC_ADDR_DISCONTINUOUS, RCC_DEST_ADDR_DISCONTINUOUS);//??

}

void HorLineN(SHORT x1, SHORT x2, SHORT y)
{
	...
	{
	    width = x1 - x2 + 1;
        srcOffsetAddr = (y * (DWORD)DISP_HOR_RESOLUTION) + x2;
	}	
	else 
	{
	    width = x2 - x1 + 1;
        srcOffsetAddr = (y * (DWORD)DISP_HOR_RESOLUTION) + x1;
	}
    ...
    //GFX_RCC_SetColor(GetColor());
	GFX_RCC_SetDestOffset(srcOffsetAddr);
	GFX_RCC_SetSize(width,1);
	GFX_RCC_StartCopy(RCC_SOLID_FILL, RCC_ROP_C, RCC_SRC_ADDR_DISCONTINUOUS, RCC_DEST_ADDR_DISCONTINUOUS);//??

}
			   
void OutCharN(UXCHAR ch)
{ 
    while(IsDeviceBusy() != 0) {Nop();}
	...
	MoveTo(DrvGetX()+1, DrvGetY());
	//_i=_i+10;

}

void TextOutN(SHORT _x, SHORT _y, UXCHAR *_str, DWORD _addr)
{ 
   UXCHAR   _ch;
   MoveTo(_x, _y);
...
{
	GFX_WaitForCommandQueue(GFX_COMMAND_QUEUE_LENGTH);
	GFX_WaitForGpu();//while (GFX_IsPuGpuBusy());

	GFX_SetDisplayArea(GFX_WORK_AREA2_START_ADDRESS);
...
/*void Swap(BYTE _flag)
{
   if(_flag==1)
   
   {
     //_DisplayBuffer=GFX_WORK_AREA2_START_ADDRESS;
    //_FrameBuffer=GFX_WORK_AREA1_START_ADDRESS;
  ...
  // GFX_SetDisplayArea(_DisplayBuffer);

  //  GFX_SetWorkArea1(_DisplayBuffer);
  //  GFX_SetWorkArea2(_FrameBuffer);
    ...
    else 
   {
    ...
    GFX_SetDisplayArea(GFX_WORK_AREA1_START_ADDRESS);
...
WORD PMPLineBarX(SHORT left, SHORT top, SHORT right, SHORT bottom,BYTE _flag)
{
...
/*        width   =   right - left + 1;
        ...
        if(_flag==1)
        {GFX_SetWorkArea2(GFX_WORK_AREA2_START_ADDRESS);}
		else {GFX_SetWorkArea2(GFX_WORK_AREA1_START_ADDRESS);}
		//(GFX_WORK_AREA2_START_ADDRESS); 
        GFX_WaitForCommandQueue(4);
		//GFX_WaitForCommandQueue(GFX_COMMAND_QUEUE_LENGTH);
        GFX_RCC_SetColor(GetColor());
...
*/
///////////////////////////////////////////////////////////
/*void PMPPutPixel(SHORT x, SHORT y,BYTE _flag)
{
